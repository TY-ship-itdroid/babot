...
async def on_message(message):
    ...
    if message.content == '!ping':
        ...
        format_response = format_claude.messages.create(
            ...
            max_tokens=100,
            system='ブルーアーカイブのキャラ名を正式名称に変換してください。入力が「衣装名+キャラ名」の形式なら「キャラ名（衣装名）」に変換してください。例：水着ナグサ→ナグサ（水着）、私服ホシノ→ホシノ（私服）、アイドルコハル→コハル（アイドル）。変換後の名前だけを返してください。変換不要な場合はそのまま返してください。キャラ名以外の文章が含まれる場合はキャラ名だけを抽出して変換してください。',
            messages=[
